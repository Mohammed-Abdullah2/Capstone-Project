...
df_clean = pd.read_csv("Data/youtube_data_full_clean.csv")
def load_lot2(url):
    r= requests.get(url)
...
col0,col1, col2,col6, col3,col4,col7,col5 = st.columns([0.7,2,1,0.7,2,1,0.7, 2])  
with col0:
    st.write('')
# First column
with col1:
...
result = pd.DataFrame({
    "Year": mycolumns,  
    "Total": df_clean[mycolumns].sum(axis=0)  
})
...
fig.add_shape(
    ...
    y0=specific_value - 1000000000, y1=specific_value + 1000000000,  
    line_color="black",
    line_width=2,
)
specific_year = 2020
...
st.write(category_texts[selected_category])  
...
fig.update_layout(
    title=dict(
        text=f"مشاهدات الفئة: {selected_category}", 
        font=dict(size=10),
        x=0.5, y=0.9,
...
st.markdown("""
           اي فرد بيفتح قناة بيكون يعتمد على نفسه او شركه  في صناعة المحتوى من الشركات الي داخله مجال اليوتيوب مثل روتانا في الاغاني و روتانا خليجيه في مسلسلات الخليجيه.  هل هذا الشي يمنعك انك تدخل كفرد في اليوتيوب؟ لا, عندك اكبر مثال بندريتا معتمد على نفسه  
""",unsafe_allow_html=True)
...
fig.update_layout(
    ...
    xaxis=dict(title="القنوات",title_standoff=40,title_font=dict(weight="bold")),  
...
result = pd.DataFrame({
    "Year": mycolumns,  
    "Total": df_bandr[mycolumns].sum(axis=0)  
})
result["Year"]= result["Year"].str.replace("_views", "")
result["Year"] = pd.to_datetime(result["Year"],format="%Y").dt.year
fig = px.line(
